...
namespace SimpleUnityCheatConsole
{
    public class DebugConsole : MonoBehaviour
    {
        ...
        private readonly CheatCommandExecutor _cheatCommandExecutor = new CheatCommandExecutor();
        ...
        {
            _consoleRect = new Rect(0, Screen.height - GetConsoleRectHeight(), Screen.width, GetConsoleRectHeight());
            _inputRect = new Rect(0, Screen.height - TextInputFieldHeight, Screen.width, TextInputFieldHeight);

            _logsRect = new Rect(0, Screen.height - _consoleRect.height + TopSpacing, Screen.width,
                _consoleRect.height - TopSpacing - TextInputFieldHeight - TextInputFieldSpacing);
            _logsContentRect = new Rect(0, 0, Screen.width, _logs.Count * LogHeight);

            _clearLogButtonRect =
                new Rect(Screen.width - 100 - 120, Screen.height - _consoleRect.height, 100, TopSpacing);
            _toggleLogButtonRect = new Rect(Screen.width - 100, Screen.height - _consoleRect.height, 100, TopSpacing);

            _suggestionsRect = new Rect(0, Screen.height - GetSuggestionsHeight() - TextInputFieldHeight,
                Screen.width / 2, GetSuggestionsHeight());
            _suggestionsContentRect =
                new Rect(0, 0, _suggestionsRect.width, _possibleCommands.Length * SuggestionHeight);
        }

        private int GetConsoleRectHeight() =>
            Mathf.Min(
                TextInputFieldSpacing + TextInputFieldHeight + (_showLog ? (_logs.Count * LogHeight) : 0) + TopSpacing,
                Screen.height / 2);

        private int GetSuggestionsHeight() => Mathf.Min(_possibleCommands.Length * SuggestionHeight, Screen.height / 2);

        [Button]
        private void TestLog(string message) => Debug.Log(message);

        [Button]
        private void TestLogError(string message) => Debug.LogError(message);

        [Button]
        private void TestLogWarning(string message) => Debug.LogWarning(message);
        ...
        private void OnGUI()
        {
            ...
            _possibleCommands = _cheatCommandExecutor.GetPossibleCommands(_userInput);
        ...
        private void DrawConsole() => GUI.Box(_consoleRect, "Console");

        private void DrawConsoleInput()
        {
            bool validCommand = _cheatCommandExecutor.IsValidCommand(_userInput);
        ...
        {
            _scrollPositionLogs = GUI.BeginScrollView(_logsRect, _scrollPositionLogs, _logsContentRect, false, false,
            ...
            for (int i = 0; i < _logs.Count; i++)
            {
                ...
                {
                    GUI.TextArea(new Rect(0, _logsContentRect.height - LogHeight * (i + 1), Screen.width, LogHeight),
                        log.Message);
                }
            }
        ...
        private void DrawConsoleButtons()
        {
        ...
        private void HandleInput() => _cheatCommandExecutor.Execute(_userInput);

        private void DrawSuggestions()
        {
            GUI.Box(_suggestionsRect, "Suggestions");

            _scrollPositionSuggestions = GUI.BeginScrollView(_suggestionsRect, _scrollPositionSuggestions,
                _suggestionsContentRect, false, false, GUIStyle.none, GUI.skin.verticalScrollbar);
            ...
            for (int i = 0; i < _possibleCommands.Length; i++)
            {
                ...
                GUI.TextArea(
                    new Rect(0, _suggestionsContentRect.height - SuggestionHeight * (i + 1),
                        _suggestionsContentRect.width, SuggestionHeight), stringBuilder.ToString());
            }
